...
public class Index : IDisposable {
	...
	/// <summary>
	/// Number of directory records stored in this index (directory blob metadata).
	/// </summary>
	public int DirectoryRecordCount => _Directories.Length;
	/// <summary>
...
#pragma warning restore CS1734
	...
	/// <summary>
	/// Directory-bundle path entries that could not be matched to a <see cref="FileRecord"/> during the last <see cref="ParsePaths"/> call.
	/// </summary>
	public IReadOnlyList<ParsePathFailure> ParsePathFailures { get; private set; } = [];

	/// <summary>
	/// <see cref="FileRecord"/> entries that still have no <see cref="FileRecord.Path"/> after the last <see cref="ParsePaths"/> call.
	/// </summary>
	public IReadOnlyList<UnresolvedFileRecord> UnresolvedFiles { get; private set; } = [];

	/// <summary>
	/// Human-readable description of the path hashing algorithm detected from this index.
	/// </summary>
	public string NameHashAlgorithmDescription => _Directories.Length == 0 ? "Unknown (no directory records)" : _Directories[0].PathHash switch {
		0xF42A94E69CFF42FEul => "MurmurHash64A (PoE 3.21.2+, seed 0x1337B33F)",
		0x07E47507B4A92E53ul => "FNV-1a 64-bit (pre-PoE 3.21.2)",
		var h => $"Unknown (DirectoryRecord[0].PathHash = 0x{h:X16})"
	};
	...
	public virtual unsafe int ParsePaths() {
		EnsureNotDisposed();
		if (pathsParsed)
			return ParsePathFailures.Count;
		...
		var failed = 0;
		var failures = new List<ParsePathFailure>();
		fixed (byte* p = directory) {
			for (var dirIndex = 0; dirIndex < _Directories.Length; ++dirIndex) {
				var d = _Directories[dirIndex];
							...
							if (Base)
								...
								fixed (byte* bp = b) {
									...
									if (_Files.TryGetValue(hash, out var f))
										...
										++failed;
										failures.Add(new ParsePathFailure {
											DirectoryRecordIndex = dirIndex,
											DirectoryPathHash = d.PathHash,
											DirectoryDataOffset = d.Offset,
											DirectoryDataSize = d.Size,
											AttemptedPath = new string((sbyte*)bp, 0, b.Length),
											ComputedPathHash = hash,
											Kind = ParsePathFailureKind.PathHashNotInFileTable,
											FromConcatenatedSegments = true
										});
									}
								}
							}
							...
							if (Base)
								...
								if (_Files.TryGetValue(hash, out var f))
									...
									failures.Add(new ParsePathFailure {
										...
										DirectoryDataOffset = d.Offset,
										DirectoryDataSize = d.Size,
										AttemptedPath = new string((sbyte*)ptr, 0, str.Length),
										ComputedPathHash = hash,
										Kind = ParsePathFailureKind.PathHashNotInFileTable,
										FromConcatenatedSegments = false
									});
								}
							}
						}
		...
		var unresolved = new List<UnresolvedFileRecord>();
		foreach (var f in _Files.Values) {
			if (f.Path is null)
				unresolved.Add(new UnresolvedFileRecord {
					PathHash = f.PathHash,
					BundlePath = f.BundleRecord.Path,
					BundleIndex = f.BundleRecord.BundleIndex,
					Offset = f.Offset,
					Size = f.Size
				});
		}
		ParsePathFailures = failures;
