...
### Fixed
- 트리 항목 **전체 줄**이 선택·하이라이트되던 문제 — 파일명(아이콘+이름) 영역만 클릭 가능, 오른쪽 빈 공간은 선택 안 됨
...
### Improved
- **Hide(제외 키워드)** 필터 반응 속도 개선 — 하위 폴더 가시성 캐시, 펼쳐진 가지만 UI 갱신, 입력 지연 100ms
...
### Fixed
- 트리 **단일 클릭** 시 이전 다중 선택 하이라이트가 남던 문제
- **Ctrl/Shift 없이** 드래그만으로 범위 다중 선택되던 문제 (Shift+드래그만 범위 선택)
- 빈 공간 드래그 시 선택될 항목 **미리보기 하이라이트** + 마퀴 박스 표시
- GGPK 트리 ↔ 번들 트리 간 선택 상호 초기화
...
### Fixed
- 트리 선택 하이라이트가 흰색·검은 글씨로 튀던 문제 — 다크 테마에 맞는 남색 톤 + 밝은 글씨로 통일 (즐겨찾기·GGPK·번들 트리 공통)
...
[2.9.33]: https://github.com/Baegovda/GGPK_Custom/releases/tag/v2.9.33
[2.9.32]: https://github.com/Baegovda/GGPK_Custom/releases/tag/v2.9.32
